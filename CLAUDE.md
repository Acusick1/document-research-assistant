...
- **Vector DB**: `qdrant-client`. Local mode (`path="./qdrant_data"`) for dev. Switching to Docker/Cloud is config-only via `RA_QDRANT_MODE`.
...
- **LLM (generation)**: `anthropic:claude-sonnet-4-6`
- **LLM (eval judge)**: `openai:gpt-4o-mini` — must be a different model family from generation to avoid systematic bias.
...
- **Config via pydantic-settings.** All environment-specific values (API keys, Qdrant mode, model names, top_k) come from `Settings` loaded from env vars with `RA_` prefix. No hardcoded values in application code.
